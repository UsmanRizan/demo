import { count } from "drizzle-orm";

import { requireAdmin } from "@/lib/admin";
import Header from "@/components/Header";
import Footer from "@/components/Footer";
import SetPasswordPrompt from "@/components/SetPasswordPrompt";
import { db } from "@/lib/prisma";
import { users, sports, bookings } from "@/db/schema";

export default async function AdminDashboard() {
  const user = await requireAdmin();

  const [totalUsers, totalSports, totalBookings] = await Promise.all([
    db.select({ value: count() }).from(users),
    db.select({ value: count() }).from(sports),
    db.select({ value: count() }).from(bookings),
  ]);

  return (
    <main className="min-h-screen bg-white">
      <Header user={user} />

      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
        {/* Welcome */}
        <div className="mb-8">
          <p className="text-sm font-bold uppercase text-gray-500">Admin Dashboard</p>
          <h1 className="mt-1 text-2xl font-bold uppercase sm:text-3xl">Administration</h1>
        </div>

        {/* Stats */}
        <div className="mb-8 grid gap-4 sm:grid-cols-3">
          <div className="border-[2px] border-black bg-white p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500 uppercase font-bold">Total Users</p>
                <p className="mt-1 text-2xl font-bold">{totalUsers[0].value}</p>
              </div>
              <div className="flex h-10 w-10 items-center justify-center border-[2px] border-black bg-black text-white">
                <svg className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                  <path strokeLinecap="square" strokeLinejoin="miter" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
                </svg>
              </div>
            </div>
          </div>

          <div className="border-[2px] border-black bg-white p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500 uppercase font-bold">Sports</p>
                <p className="mt-1 text-2xl font-bold">{totalSports[0].value}</p>
              </div>
              <div className="flex h-10 w-10 items-center justify-center border-[2px] border-black bg-black text-white">
                <svg className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                  <path strokeLinecap="square" strokeLinejoin="miter" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" />
                </svg>
              </div>
            </div>
          </div>

          <div className="border-[2px] border-black bg-white p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500 uppercase font-bold">Bookings</p>
                <p className="mt-1 text-2xl font-bold">{totalBookings[0].value}</p>
              </div>
              <div className="flex h-10 w-10 items-center justify-center border-[2px] border-black bg-black text-white">
                <svg className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                  <path strokeLinecap="square" strokeLinejoin="miter" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" />
                </svg>
              </div>
            </div>
          </div>
        </div>

        {/* Quick Actions */}
        <h2 className="text-lg font-bold uppercase">Management</h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <a
            href="/admin/users"
            className="group flex items-center gap-4 border-[2px] border-black bg-white p-6 transition-all hover:bg-black hover:text-white"
          >
            <div className="flex h-12 w-12 items-center justify-center border-[2px] border-black bg-black text-white transition-colors group-hover:bg-white group-hover:text-black">
              <svg className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path strokeLinecap="square" strokeLinejoin="miter" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
              </svg>
            </div>
            <div>
              <h3 className="text-base font-bold uppercase">Manage Users</h3>
              <p className="mt-0.5 text-sm text-gray-500 group-hover:text-gray-400">
                View, create, and manage user accounts and roles.
              </p>
            </div>
            <svg className="ml-auto h-5 w-5 text-gray-400 transition-colors group-hover:text-white" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
              <path strokeLinecap="square" strokeLinejoin="miter" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
            </svg>
          </a>

          <a
            href="/admin/sports"
            className="group flex items-center gap-4 border-[2px] border-black bg-white p-6 transition-all hover:bg-black hover:text-white"
          >
            <div className="flex h-12 w-12 items-center justify-center border-[2px] border-black bg-black text-white transition-colors group-hover:bg-white group-hover:text-black">
              <svg className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path strokeLinecap="square" strokeLinejoin="miter" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" />
              </svg>
            </div>
            <div>
              <h3 className="text-base font-bold uppercase">Manage Sports</h3>
              <p className="mt-0.5 text-sm text-gray-500 group-hover:text-gray-400">
                Add, activate, or deactivate sports categories.
              </p>
            </div>
            <svg className="ml-auto h-5 w-5 text-gray-400 transition-colors group-hover:text-white" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
              <path strokeLinecap="square" strokeLinejoin="miter" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
            </svg>
          </a>

          <a
            href="/admin/withdrawals"
            className="group flex items-center gap-4 border-[2px] border-black bg-white p-6 transition-all hover:bg-black hover:text-white"
          >
            <div className="flex h-12 w-12 items-center justify-center border-[2px] border-black bg-black text-white transition-colors group-hover:bg-white group-hover:text-black">
              <svg className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path strokeLinecap="square" strokeLinejoin="miter" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" />
              </svg>
            </div>
            <div>
              <h3 className="text-base font-bold uppercase">Withdrawals</h3>
              <p className="mt-0.5 text-sm text-gray-500 group-hover:text-gray-400">
                Review and manage owner withdrawal requests.
              </p>
            </div>
            <svg className="ml-auto h-5 w-5 text-gray-400 transition-colors group-hover:text-white" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
              <path strokeLinecap="square" strokeLinejoin="miter" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
            </svg>
          </a>
        </div>
      </div>

      <Footer />
      <SetPasswordPrompt />
    </main>
  );
}
